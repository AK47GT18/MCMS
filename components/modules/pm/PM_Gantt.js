...
,
    ...
    async renderGanttChart(retryCount = 0) {
        ...
        try {
            ...
            style.innerHTML = `
                #gantt { display:block !important; width:100%; }
                #gantt-chart-container { display:block !important; width:100% !important; overflow:auto !important; position:relative; max-height:80vh; }
                .gantt-container { display:block !important; width:100%; overflow:visible !important; }
            ...
            try {
                this.ganttInstance = new GanttCls("#gantt", mappedTasks, {
                    ...
                    custom_popup_html: () => ''   // disable native popup completely
                });
            ...
            setTimeout(() => {
                ...
                // Fix SVG height to fit all rows
                const neededH = HEADER_H + mappedTasks.length * ROW_H + 40;
                svg.setAttribute('height', Math.max(neededH, 300).toString());
                ...
                svg.addEventListener('click', (e) => {
                    document.querySelectorAll('.gantt-custom-popup').forEach(p => p.remove());
                    svg.querySelectorAll('.bar-wrapper').forEach(w => w.classList.remove('selected'));
                    const wrapper = e.target.closest('.bar-wrapper');
                    if (wrapper) wrapper.classList.add('selected');
                });

                // ── Hover popup — match by index, not label text ──
                const showPopup = (task, anchorEl) => {
                    document.querySelectorAll('.gantt-custom-popup').forEach(p => p.remove());
                    ...
                    popup.style.cssText = 'position:fixed; z-index:9999; pointer-events:none; width:280px;';
                    popup.innerHTML = `
                        <div style="padding:16px; background:white; border-radius:10px; border:1px solid #e2e8f0; font-family:system-ui,sans-serif;">
                            ...
                            ${task.dependencies ? `
                            <div style="display:flex; justify-content:space-between; align-items:center; background:${bg}; padding:8px; border-radius:6px; border:1px solid ${border};">
                                <span style="font-size:11px; font-weight:600; color:#475569;">Depends On</span>
                                <span style="font-size:12px; font-weight:700; color:#0f172a; max-width:150px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${task.custom_dependency_name}">${task.custom_dependency_name}</span>
                            </div>` : ''}
                        </div>
                    `;
                ...
                // Superior Native JS Hover Popups via Event Delegation
                let hoverPopupTimeout;
                ...
                svg.addEventListener('mouseover', (e) => {
                    ...
                    // Don't re-render if already showing for this wrapper
                    if (wrapper.classList.contains('popup-active')) return;
                    
                    document.querySelectorAll('.gantt-custom-popup').forEach(p => p.remove());
                    svg.querySelectorAll('.bar-wrapper').forEach(w => w.classList.remove('popup-active'));
                    wrapper.classList.add('popup-active');
                    
                    // Find task by index matching
                    const allWrappers = Array.from(svg.querySelectorAll('.bar-wrapper'));
                    const idx = allWrappers.indexOf(wrapper);
                    const task = mappedTasks[idx];
                    if (!task) return;

                    showPopup(task, wrapper);
                });
                ...
                svg.addEventListener('mouseout', (e) => {
                    ...
                    // If moving to a child within the same wrapper, ignore
